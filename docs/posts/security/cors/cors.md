...
Cross-Origin Resource Sharing의 줄임말로, 서버 origin과 현재 클라이언트에 표시된 페이지의 origin(URL protocol + domain + port)이 다른 상황에서 리소스를 주고받기 위한 HTTP 헤더 기반 메커니즘입니다.

보통 클라이언트 페이지에서 XMLHttpRequest 또는 Fetch API로 리소스를 요청할 때, SOP(Same-Origin Policy) 정책에 의해서 origin 값이 동일할 서버로만 요청을 전송할 수 있습니다.
...
3. 클라이언트에서 요청 origin과 `Access-Control-Allow-Origin`에 명시된 origin을 비교했을 때 동일하지 않다면 응답 데이터의 status code가 `200 OK여도 바로 폐기합니다.
4. 만일 origin 값이 동일하다면 actual request를 서버로 전송합니다.
...
클라이언트에서 서버로 본 요청(actual request)를 전송하기 전에 [OPTIONS 메서드](../../http/http1/methods#options)를 사용한 예비 요청(preflight request)을 전송하여 서버가 허용하는 요청 설정(?)을 확인합니다.

#### 가능한 요청헤더들

- `Origin` : 요청을 전송하는 클라이언트의 origin
- `Access-Control-Request-Method` : 본 요청에서 사용할 HTTP 메서드 타입
- `Access-Control-Request-Headers` : 본 요청에서 사용할 HTTP 헤더(들)
...
- `Access-Control-Allow-Credentials` : 본 요청에서 인증과 관련된 정보를 담을 수 있는지 여부
- `Access-Control-Allow-Methods` : 본 요청에서 사용가능한 HTTP 메서드(들)
- `Access-Control-Allow-Headers` : 본 요청에서 사용할 수 있는 헤더(들)
...
예비요청 없이 클라이언트에서 본 요청이 서버로 전송되면 브라우저에서 서버의 `Access-Control-Allow-Origin`과 본 요청의 Origin을 비교하여 CORS 정책 위반 여부를 검사하는 방식입니다.

간단하다는 장점이 있지만 특정 조건(요청 메서드, 요청 헤더, content-type 등)을 만족하는 경우에만 예비 요청을 생략할 수 있다는 까다로운 점이 있어서 실제 실무에서는 잘 사용되지 않습니다.
...
credential 옵션으로 가능한 값으로 3가지가 있다.

- `same-origin(기본값)` : 같은 origin 간의 요청에만 인증정보를 담을 수 있습니다.
- `include` : 모든 요청에 인증정보를 담을 수 있습니다.
- `omit` : 모든 요청에 인증정보를 담지 않습니다.

credential을 담은 요청 메시지를 전송하여 서버로부터 응답이 오면 브라우저는 응답 메시지로부터 다음 2가지를 확인합니다.

- `Access-Control-Allow-Origin` 헤더값으로 "\*"가 아닌 명시적인 URL인가?
...
API 서버 응답 메시지의 `Access-Control-Allow-Origin` 헤더에 알맞은 값을 셋팅하고 요청 헤더에도 적절한 `Origin` 값을 셋팅합니다.
